...
- **Fotos na aba Início (24/07/2026)**: a pedido do Ezequiel, pra ficar mais parecido com o mockup. `MODULE_IMAGES` no `server.js` reaproveita a mesma foto (Unsplash) que cada módulo já usa internamente — não baixou nem escolheu nada novo. Aplicado em 3 lugares: atalhos da Início (círculo com foto em vez de emoji), card "Continue de onde parou" (foto quadrada arredondada) e os cards de módulo da aba Conteúdos (foto grande no topo + linha dourada no rodapé, como os cards do mockup). **Bug pego durante o teste local**: o card "Continuar" é montado por JS no navegador (não no servidor), e a primeira tentativa de colocar a foto quebrou por causa de aspas mal escapadas (JS dentro de JS dentro de template literal — 3 níveis) — trocado pra usar `&quot;` em vez de tentar escapar aspas simples, resolveu.
...
- Assinados como "Equipe O Código da Longevidade" (não em nome do Valdoir — decisão do Ezequiel de 23/07, já que ele quem responde de verdade e o Valdoir ainda nem sabe do produto), tom acolhedor, sem prometer cura/resultado — mesma linha de cuidado do restante do conteúdo (ver [[feedback_evidence_graded_content]]).
...
- **Hero card na aba Início (24/07/2026)**: a pedido do Ezequiel, pra ficar igual ao mockup. Card no topo da Início com foto (silhueta ao pôr do sol, Unsplash `photo-1506126613408`, verificada 200 antes de usar), a frase "Pequenas escolhas, grandes transformações." (que o Ezequiel curtiu no mockup), subtítulo e botão "Comece por aqui →" que aponta pro mesmo ponto de continuação do card Continuar. Substituiu o hero de texto simples ("Sua biblioteca / Logado como..."). Traduzido PT/ES/EN. Mantido também o card "Continue de onde parou" abaixo (o hero é aspiracional, o card mostra o módulo específico).
